...
## model3: refit model2_sqrt without outliers this time
**p=210**, n = 424, R2= 0.6584, R2a= 0.3247
```{r}
model3 = lm(avg_salary_k~(.)^2, data = df_out)
summary(model3)
```
...
## model3_aic: AIC of model3
```{r}
fit0 = lm(avg_salary_k~1, data=df_out)
step.aic = stepAIC(fit0, scope=list(upper=model3, lower=fit0), trace=FALSE)
step.aic$anova
```

R2 = 0.4125, R2a = 0.3833

```{r}
model3_aic = lm(avg_salary_k ~ senior_status + in_CA + python + visual_software + 
...
plot(model3_aic)
```
##Confidence Interval for model3_aic
```{r}
#Confidence Interval
